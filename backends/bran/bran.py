...
def process(form):
    ...
    if "variable" in form and "date" in form and "period" in form and "area" in form:
        ...
        if mapStr == 'anom':
            ...
            fileName = decGraph % (branProduct["monthlyDec"], areaStr, dateStr[:6])
        ...
        ####current xml html response
            if periodStr == 'daily':
                ...
                fileName = avebranGraph % (branProduct["yearly"], mapStr, areaStr, dateStr[:4])
            elif periodStr == '3monthly':
                fileName = avebranGraph % (branProduct["3monthly"], mapStr, areaStr, dateStr[:6])
            elif periodStr == '6monthly':
                fileName = avebranGraph % (branProduct["6monthly"], mapStr, areaStr, dateStr[:6])
            elif periodStr == 'weekly':
                fileName = avebranGraph % (branProduct["weekly"], mapStr, areaStr, dateStr)
            outputFileName = serverCfg["outputDir"] + fileName
            if not os.path.exists(outputFileName + ".png"):
